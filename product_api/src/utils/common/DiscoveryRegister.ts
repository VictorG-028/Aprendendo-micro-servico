...
export default class DiscoveryRegister {
  ...
  public static async registerService(): Promise<boolean> {
    try {
      ...
      await axios.post(`${envVars.DISCOVERY_URL}/register`, {
      ...
      console.error(`Failed to register service using DISCOVERY_URL=${envVars.DISCOVERY_URL} CATEGORY=${envVars.CATEGORY}  SERVICE_URL=${envVars.SERVICE_URL}. Error msg: ${axiosError.message}`);
